...
- 站点可启用和排序，安装时一次性授权四个内置 AI 站点；默认优先级为 Gemini、ChatGPT、豆包、Kimi。
- 每个站点可选择“自动发送”或“仅填入”，并可指定一个优先会话 URL。
- 扇贝页面快捷键可在设置页直接录入，默认是键盘左上角的 <kbd>` / ·</kbd> 键。
- 只选择已启用、已打开的站点；不会擅自打开网页。
...
4. 首次安装会打开设置页。浏览器会一次性显示四个内置 AI 站点的权限，确认后按需启用站点并保存。
...
- 扇贝和四个内置 AI 域名均为安装时声明的站点权限；浏览器会统一展示并由用户确认。
- 设置页的站点开关决定是否注册对应内容脚本，停用的站点不会参与调度。
